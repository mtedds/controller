...
class Shelly:
    ...
    def __init__(self, in_message_handler, in_database, inLogger):
        ...
        self.gateway_id = self.database.gatewayFindFromSubscribeTopic("shelly2")["GatewayId"]
    ...
    def presentation(self):
        ...
        self.message_handler.publish(f"shelly2/{self.node_id}/255/0/0/18", software_version)

        # Announce the "sketch" and "version"
        self.message_handler.publish(f"shelly2/{self.node_id}/255/3/0/11", "Shelly EM - shelly2")
        self.message_handler.publish(f"shelly2/{self.node_id}/255/3/0/12", software_version)

        # Announce all of the sensors
        for sensor in self.sensors.keys():
            self.message_handler.publish(
                f"shelly2/{self.node_id}/{self.sensors[sensor]['id']}/0/0/{self.sensors[sensor]['type']}",
                self.sensors[sensor]['name'])

    def discover_response(self):
        self.logger.debug(f"shelly discover_response")
        self.message_handler.publish(f"shelly2/{self.node_id}/255/{COMMAND_INTERNAL}/0/{I_DISCOVER_RESPONSE}",
                                     self.node_id)
